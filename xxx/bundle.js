		...
		methods: {
			...
			getSongs: function getSongs() {
				...
				var opts = {
					...
					success: function success(responseText) {
						...
						_this.pic = response.picture;
						// this.lrc = response.lrc;//只是链接
						_this.sid = response.sid;
						_this.title = response.title;
						_this.url = response.url;

						document.getElementsByTagName('article')[0].style.backgroundImage = 'url(' + _this.pic + ')';
						var song = document.getElementsByTagName('audio')[0];

						var clock = setInterval(function () {
							if (song.duration) {
								clearInterval(clock);
								_this.allcnt = song.duration;
								_this.endTime = _this.dealWithSeconds(_this.allcnt);
						...
						setTimeout(_this.playSong, 2000);
					},
			...
			getLyric: function getLyric() {
				...
				var opts = {
					...
					success: function success(responseText) {
						var response = JSON.parse(responseText);
						_this2.lyric = response.lyric;
						_this2.dealWithLyric(response.lyric);
					},
					error: function error() {
			...
			dealWithLyric: function dealWithLyric(lyrictext) {
			...
			dealWithSeconds: function dealWithSeconds(seconds) {
			...
			playSong: function playSong() {
				...
				this.clock = setInterval(function () {
					_this3.cnt++;
					_this3.nowTime = _this3.dealWithSeconds(_this3.cnt);
					if (!!_this3.lyricArr[_this3.nowTime]) {
						_this3.arr.shift();
						if (_this3.setLyric) {
							for (var key in _this3.arrTemp) {
								if (_this3.arrTemp[key].time === _this3.dealWithSeconds(_this3.cnt)) {
									var idx = _this3.arrTemp.indexOf(_this3.arrTemp[key]);
									_this3.arr = _this3.arrTemp.slice(idx - 4, _this3.arrTemp.length);
									_this3.setLyric = false;
									break;
								}
							}
						}
					...
					ele.style.width = _this3.cnt / _this3.allcnt * 320 + 'px';
			...
			moveVolume: function moveVolume() {
			...
			toggleNoVolume: function toggleNoVolume() {
				var node = document.getElementsByClassName('nowvolume')[0];
				var song = document.getElementsByTagName('audio')[0];
